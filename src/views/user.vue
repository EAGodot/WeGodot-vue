...
export default {
  ...
  data() {
    return {
      ...
      // ... 其他数据 ...
      registrationCode: '', // 注册序列号20251114
  ...
  methods: {
    ...
    register() {
      ...
      if (
        ...
        this.$common.isEmpty(this.password) ||
        this.$common.isEmpty(this.registrationCode)  // 新增：验证序列号
      ) {
        this.$notify({
          type: "error",
          title: "可恶🤬",
          message: "请输入用户名、密码和注册序列号！",
          position: "top-left",
          offset: 50,
        });
        return;
      ...
      if (
        ...
        this.registrationCode.indexOf(" ") !== -1  // 新增：序列号也不能包含空格
      ) {
        this.$notify({
          type: "error",
          title: "可恶🤬",
          message: "用户名、密码或序列号不能包含空格！",
          position: "top-left",
          offset: 50,
        });
        return;
      ...
      let user = {
        ...
        password: this.$common.encrypt(this.password.trim()),
        registration_code: this.registrationCode.trim(),  // 新增：序列号
      };
      ...
      this.$http
        ...
        .catch((error) => {
          if (error.response && error.response.data) {
            ...
            } else if (errorData.error === "注册序列号无效或已被使用。") {
              this.$notify({
                type: "error",
                title: "可恶🤬",
                message: "注册序列号无效或已被使用！",
                position: "top-left",
                offset: 50,
              });
            } else if (errorData.error === "请提供用户名、密码和注册序列号。") {
              this.$notify({
                type: "error",
                title: "可恶🤬",
                message: "请填写完整的注册信息！",
    ...
    submitDialog() {
      if (this.dialogTitle === "修改头像") {
        if (this.$common.isEmpty(this.avatar)) {
          ...
          console.log('🔄 提交頭像修改，頭像URL:', user.avatar);
          ...
          this.$http
            ...
            .then((res) => {
              if (!this.$common.isEmpty(res.result[0])) {
                const userData = res.result[0].data[0];
                console.log('📥 服務器返回的用戶數據:', userData);
                
                // 🔧 修復：處理頭像URL後再提交到Vuex
                this.processAvatarForStore(userData);
                
                this.$store.commit("loadCurrentUser", userData);
                this.currentUser = this.$store.state.currentUser;
                
                // 🔧 新增：強制刷新頭像顯示
                this.$nextTick(() => {
                  this.forceUpdateAllAvatars();
                });
    ...
    // 修復 processAvatarForStore 方法
    processAvatarForStore(userData) {
      if (userData.avatar) {
        ...
        if (avatarUrl.includes('/api/')) {
          ...
          console.log('🔄 移除 /api/ 前綴:', avatarUrl);
        }
        ...
        if (!avatarUrl.startsWith('http')) {
          ...
          userData.avatar = mediaBaseURL + avatarUrl;
        }
        
        // 添加時間戳
        const separator = userData.avatar.includes('?') ? '&' : '?';
        userData.avatar = userData.avatar + separator + 't=' + new Date().getTime();
        
        console.log('✅ 處理後的頭像URL:', userData.avatar);
      }
    },
    ...
    forceUpdateAllAvatars() {
      console.log('🔄 強制更新所有頭像顯示');
      
      // 方法1: 直接更新DOM中的頭像元素
      const updateDomAvatars = () => {
        ...
        avatarSelectors.forEach(selector => {
          ...
          elements.forEach((img, index) => {
            if (this.currentUser.avatar) {
              // 使用新的時間戳
              const newSrc = this.currentUser.avatar.split('?')[0] + '?force=' + new Date().getTime();
              console.log(`更新DOM頭像 ${index + 1}:`, newSrc);
              img.src = newSrc;
            }
          });
        });
...
.in-up {
  ...
  position: relative;
  overflow: hidden;
  width: 800px; /* 從 750px 增加到 800px */
  max-width: 95%; /* 從 100% 改為 95%，讓兩邊有更多空間 */
  min-height: 480px; /* 從 450px 增加到 480px */
  margin: 20px auto; /* 增加上下邊距，改為居中 */
  p {
    font-size: 14px;
    letter-spacing: 1px;
  ...
  button {
    ...
    padding: 12px 45px;
    letter-spacing: 2px;
    margin: 10px 0; /* 增加按鈕上下邊距 */
    width: 80%; /* 限制按鈕寬度 */
    max-width: 280px; /* 設置最大寬度 */
    &:hover {
...
.form-container {
  ...
  div {
    ...
    padding: 0 30px; /* 從 20px 增加到 30px，增加內邊距 */
    height: 100%;
  ...
  input {
    ...
    border: none;
    padding: 12px 15px;
    margin: 12px 0; /* 增加輸入框間距 */
    width: 100%;
    max-width: 320px; /* 限制輸入框最大寬度 */
    outline: none;
